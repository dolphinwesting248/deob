...
function generateReport(before, after) {
  ...
  const beforeVals = metrics.map((m) => m.before);
  const afterVals = metrics.map((m) => m.after);
...
new Chart(document.getElementById('chart'), {
  ...
  data: {
    ...
    datasets: [
      { label: 'Before', data: ${JSON.stringify(beforeVals)}, backgroundColor: '#475569', borderRadius: 4 },
      { label: 'After',  data: ${JSON.stringify(afterVals)},  backgroundColor: '#6366f1', borderRadius: 4 },
    ]
  },
  options: {
    responsive: true,
