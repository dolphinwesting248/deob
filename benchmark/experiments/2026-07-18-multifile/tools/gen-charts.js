#!/usr/bin/env node
// Generate SVG charts from final-scores.json
const fs = require("fs");
...
if (!fs.existsSync(SCORES_PATH)) {
  console.log("No final-scores.json found. Run score.js --all first.");
  process.exit(0);
}
...
const DIM_KEYS = ["filePurposes", "dependencies", "dataFlow", "sharedResources", "entry", "token", "time"];
const DIM_LABELS = ["File Purposes", "Dependencies", "Data Flow", "Shared Res", "Entry", "Token", "Time"];
const W = 420, H = 320;
...
function svgRadar(scenario) {
  ...
  for (let i = 0; i < n; i++) {
    const a = -Math.PI / 2 + i * angleStep;
    svg += `<line x1="${cx}" y1="${cy}" x2="${cx + Math.cos(a) * r}" y2="${cy + Math.sin(a) * r}" stroke="#eee" stroke-width="1"/>`;
  }
  ...
  for (let i = 0; i < n; i++) {
    ...
    const ly = cy + Math.sin(a) * (r + 20) + 4;
    svg += `<text x="${lx}" y="${ly}" text-anchor="middle" font-size="8">${DIM_LABELS[i]}</text>`;
  }
